...
const EMBEDDING_RETRY_DELAY_MS = 1200;
...
export async function textToEmbeddings(inputs: string[]): Promise<Embedding[]> {
  ...
  for (let attempt = 0; attempt <= EMBEDDING_MAX_RETRIES; attempt += 1) {
    try {
      ...
      if (!response.ok) {
        if ((response.status === 429 || response.status >= 500) && attempt < EMBEDDING_MAX_RETRIES) {
          await new Promise((resolve) => setTimeout(resolve, EMBEDDING_RETRY_DELAY_MS * (attempt + 1)));
          continue;
        }
