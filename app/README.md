...
A lightweight conversational art companion for multimodal art understanding, indexed semantic retrieval, and natural-language-guided art exploration. The repository includes a reproducible demo dataset and ingestion pipeline.
...
flowchart TD
    ...
    E --> F[Conversation Planner]
    F --> F1[Read thread history]
    F --> F2[Choose next action]

    F --> T[Turn Tuning Agent]
    T --> T1[Infer retrieval strictness from conversation]
    T --> T2[Adjust score floors and reranking weights]

    F --> G[Retrieval Agent]
    T --> G
    C --> G
    G --> G1[Vector candidate search]
    G --> G2[Visual + semantic + lexical scoring]
    G --> G3[Preference-aware reranking]
    G --> G4[Confidence / uncertainty labeling]

    F --> H[Research Agent]
    H --> H1[On-demand Met fetch]
    H --> H2[External context when local evidence is weak]

    G --> I[Response Agent]
    H --> I
    I --> I1[Grounded recommendations]
    I --> I2[Evidence + provenance]
    I --> I3[Saved thread + explicit gallery saves]
```
...
4. A planner interprets the turn and chooses retrieval, research, or response-with-limits.
5. Retrieval pulls candidates from the indexed corpus.
6. Research (The Met) is invoked only through planner policy and feature flags.
7. A response agent composes the final natural-language reply + evidence trail.
...
### 4) Multi-agent conversation path
- `lib/chat.ts` is now a thin orchestrator: planner -> retrieval/research -> response -> DB writes.
- `lib/conversationPlanner.ts` is pure and side-effect free. It emits `{ action, intentMode, rationale }`.
- `lib/turnTuningAgent.ts` infers per-turn retrieval tuning (score-floor deltas + weight bias) from conversation context.
- `lib/analysis.ts` runs the vision/semantic analysis agent with no DB access.
- `lib/retrieval.ts` runs corpus retrieval only (pg/pgvector, no external HTTP).
- `lib/metResearch.ts` runs external Met lookups only and is gated by `ENABLE_ON_DEMAND_MET_FETCH`.
- `lib/responseAgent.ts` owns prompt construction and final message generation.
- Planner actions and retrieval tuning are written into assistant message metadata for inspectable traces.
...
If Prisma reports `P3005` (schema not empty) or drift on an existing Neon database, baseline once before using deploy migrations:

```
npx prisma migrate resolve --applied 20260324201359_embedded_test
npx prisma migrate resolve --applied 20260324233000_companion_flow
npm run db:migrate:deploy
```

For Neon + Prisma, use:
- `DATABASE_URL` = pooled `-pooler` URL (runtime/client traffic)
- `DIRECT_URL` = non-pooled direct branch URL (Prisma migrations/introspection)
...
* `/how-it-works` — architecture and system behavior spec for the current build
* `/admin/provenance` — optional admin/debug source view
* `/admin/threads` — bulk thread management for demo cleanup
...
3. Set:
   - `DATABASE_URL` to pooled URL
   - `DIRECT_URL` to direct URL
3. Run:
...
4. Keep this split in all environments where Prisma migrations run (`DIRECT_URL` should stay direct).
...
5. Expand the evidence panels to inspect source details and the thread-level discussion trail.
...
- `npm run tags:enrich` — dry-run semantic-tag enrichment over indexed artworks (no writes)
- `npm run tags:enrich:apply` — apply semantic-tag enrichment updates to artwork metadata
...
- The thread model preserves messages, attached images, surfaced artworks, and surfaced artists separately, which makes the conversation inspectable later.
- Saved threads are distinct from the broader indexed corpus. The thread gallery now reflects works the user explicitly saved, not all surfaced recommendations.
- The assistant stays local-first by default and only queries The Met when the user explicitly accepts that next step.
...
- [lib/conversationPlanner.ts](/Users/tanvirao/multimodal-art-retrieval-demo/lib/conversationPlanner.ts)
- [lib/metResearch.ts](/Users/tanvirao/multimodal-art-retrieval-demo/lib/metResearch.ts)
